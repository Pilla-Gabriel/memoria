...
export async function GET() {
  ...
  const [statusCounts, users, extensionsCount, checkinRecovered, goalsAtingidas, goalsVencidas] = await Promise.all([
    prisma.task.groupBy({
      by: ["status"],
  ...
  const productivity = await Promise.all(
    users.map(async (u) => {
      ...
      return { id: u.id, name: u.name, completedCount, ...risk };
    })
  );
